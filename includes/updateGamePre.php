...
$handle = fopen($file, 'a');
$gamecode = $_POST['gamecode']; // coming from local storage
...
if(isset($_POST['submitting'])){
	...
	// Fetching Avatar
	$avatar = "";
	if(!is_null($_POST['user_email']) && $_POST['user_email'] !== "null"){
		$email = $_POST['user_email'];
		$sql = "SELECT `avatar` FROM `accounts` WHERE `email` = '$email' AND `active` = 1";
	...
	if($results[0] != NULL){
		...
		if (array_key_exists('num_words', $results[0])) {
		    $numwords = $results[0]['num_words'];
		}
		if (array_key_exists('deck', $results[0])) {
		    $deck = $results[0]['deck'];
		}
		if (array_key_exists('team1', $results[0])) {
		    $team1 = $results[0]['team1'];
		}
		if (array_key_exists('team2', $results[0])) {
		    $team2 = $results[0]['team2'];
		}
		if (array_key_exists('random', $results[0])) {
		    $random = $results[0]['random'];
		}
		if (array_key_exists('skip', $results[0])) {
		    $skip = $results[0]['skip'];
		}
		if (array_key_exists('virtual_play', $results[0])) {
		    $virtual = $results[0]['virtual_play'];
		}
	}

	// Inserting player with all variables into game's database:
	if($gamename == 'fishbowl'){
		$sql = "INSERT INTO `$gamename`(`game_code`, `name`, `num_words`, `team1`, `team2`, `random`, `skip`, `num_game`, `timer_time`, `virtual_play`, `avatar`, `game_state`) VALUES ('$gamecode',  '$player', '$numwords', '$team1', '$team2', '$random', '$skip', '$numgames', 60, '$virtual', '$avatar', '')";
		$mysqli->query($sql);
		fwrite($handle, $mysqli->error);
	...
	// Deleting players if name is already submitted (this is useful for when two names are submitted at the exact same time):
	$player_Array = array();
	$sql = "SELECT * FROM `$gamename` WHERE `game_code` = '$gamecode'";
	$result = $mysqli->query($sql);
	$results = NULL;
	...
	for ($x = 0; $x < count($results) - 1; $x++) {
	    $player_ind = $results[$x]['player'];
	    $player_name = $results[$x]['name'];
	    if(in_array(strtolower($player_name), $player_Array)){
	    	$sql = "DELETE FROM `$gamename` WHERE `game_code` = '$gamecode' AND `name` = '$player_name' AND `player` = '$player_ind'";
			$result = $mysqli->query($sql);
			echo FALSE;
			exit();
	    }else{
	    	array_push($player_Array, $player_name);
	    }
...
if(isset($_POST['setting_up'])){
	/*set game_state: used for resume game button*/
    $sql = "UPDATE `$gamename` SET `game_state`='starting' WHERE `game_code` = '$gamecode'";
    $mysqli->query($sql);
    /* end game state*/
    ...
    if($gamename == 'mouthful'){
    	$numteams = $_POST['num_teams'];
    	$teamnames = $_POST['teamnames'];
    	$sql = "UPDATE `$gamename` SET `num_teams`='$numteams', `team_names`='$teamnames' WHERE `game_code` = '$gamecode'";
		$mysqli->query($sql);
    }

	if($gamename == 'mouthful' || $gamename == 'gridlock'){
